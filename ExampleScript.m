...
% POTENTIALLY MEMORY EFFICIENT STRATEGY HERE %
sz = size(M1);
M1 = reshape(M1, sz(1)*sz(2),sz(3))'; % linearize pixels
M1 = pixelFilter(M1,FS,lowPass);       % Apply dF/F function
M1 = reshape(M1',sz(1),sz(2),sz(3));    % reshape the imaging matrix back into (height x width x # of frames)
...
for ii = 2:numel(dca.labNames)
    ...
    dft(ii,:) = sum(M1.*cmask,[1,2])./sum(cmask,[1,2]); % mean dF/F trace with the current mask
